    ...
    <div class="row justify-content-center">
     ...
     style="background: linear-gradient(135deg, #980517 0%, #d4142a 50%, #ff4b5c 100%);">
                    ...
                    <h1 class="text-white mb-2">
                        ...
                        Syarat & Ketentuan Wisuda
                    </h1>
                    <p class="text-white mb-0" style="opacity:0.85;">Pastikan Anda memenuhi semua persyaratan berikut sebelum mendaftar wisuda</p>
                </div>
            </div>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-body p-4 p-md-5">
                    ...
                    <div class="mt-4 text-gray">
                        ...
                        @php
                            $syaratItems = \App\Models\SyaratItem::orderBy('order_number', 'asc')->get();
                        @endphp

                        @forelse($syaratItems as $item)
                            <!-- Dynamic Item -->
                            <div class="card mb-3 border-start border-4" style="border-color:{{ $item->color ?? '#adb5bd' }};">
                                <div class="card-body">
                                    <div class="d-flex align-items-start">
                                        <div class="badge rounded-circle p-2 me-3"
                                             style="width:40px;height:40px;
                                                    display:flex;align-items:center;justify-content:center;
                                                    background-color:{{ $item->color ?? '#6c757d' }};color:#ffffff;">
                                            <strong>{{ $item->order_number }}</strong>
                                        </div>

                                        <div class="flex-grow-1">
                                            <h5 class="card-title mb-1">
                                                <i class="bi {{ $item->icon }} me-2" style="color:{{ $item->color ?? '#6c757d' }};"></i>
                                                {{ $item->title_id }}
                                            </h5>
                        ...
                        @empty
                            <p class="text-center text-muted">Belum ada persyaratan yang ditambahkan.</p>
                        @endforelse
                    </div>
