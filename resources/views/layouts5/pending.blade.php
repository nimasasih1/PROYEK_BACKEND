...
<div class="container-fluid">
    ...
    @php
        $belumCeklis = $data->where('is_valid_csdl', 0)->count();
    @endphp
        ...
        <table id="wisudaPendingTable" class="table table-bordered" style="width:100%; border-collapse: collapse;">
            <thead style="background-color: #8B0000;">
                <tr>
                    ...
                    <th style="color: white; padding: 12px 15px; text-align: center; font-weight: 600; border: 1px solid #8B0000; white-space: nowrap;" title="CSDL ✓">CSDL ✓</th>
                    <th style="color: white; padding: 12px 15px; text-align: center; font-weight: 600; border: 1px solid #8B0000; white-space: nowrap;" title="CATATAN CSDL">CSDL NOTES</th>
                    ...
                    <tr style="background-color: white;">
                        ...
                        <td style="text-align: center; padding: 15px; border: 1px solid #dee2e6;">
                            <form action="{{ route('viewmahasiswa.wisuda1.update', $pendaftaran->id_pendaftaran) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="field" value="is_valid_csdl">
                                <input type="checkbox" name="is_valid_csdl" value="1"
                                    {{ $pendaftaran->is_valid_csdl ? 'checked' : '' }}
                        ...
                        <td style="padding: 15px; border: 1px solid #dee2e6;">
                            <form action="{{ route('layouts5.validation_csdl.update1', $pendaftaran->id_pendaftaran) }}" method="POST" class="d-flex align-items-start gap-2" style="width: 500px;">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="field" value="catatan_csdl">
                                <textarea name="catatan_csdl" class="form-control" rows="2" placeholder="Write CSDL notes..." title="Tulis catatan CSDL..." style="flex:2; min-width: 350px;">{{ $pendaftaran->catatan_csdl ?? '' }}</textarea>
                                <div class="d-flex flex-column gap-1">
                                    ...
                                    <button type="button" class="btn btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#catatanCSDLModal{{ $pendaftaran->id_pendaftaran }}"
                                        title="Lihat"
                                        style="background-color: white; border: 1px solid #dee2e6; color: #8B0000;">
                            ...
                            <!-- Modal Catatan CSDL -->
                            <div class="modal fade" id="catatanCSDLModal{{ $pendaftaran->id_pendaftaran }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('layouts5.validation_csdl.update1', $pendaftaran->id_pendaftaran) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="field" value="catatan_csdl">
                                            <div class="modal-header">
                                                <h5 class="modal-title" title="Catatan CSDL">CSDL Notes</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <div class="modal-body">
                                                <textarea name="catatan_csdl" class="form-control" rows="5">{{ $pendaftaran->catatan_csdl ?? '' }}</textarea>
                                            </div>
...
<script>
    $(function() {
        ...
        // Cegah checkbox kalau catatan CSDL belum diisi
        $(document).on('change', 'input[name="is_valid_csdl"]', function(e) {
            var $checkbox = $(this);
            var $row = $checkbox.closest('tr');
            var catatan = $row.find('textarea[name="catatan_csdl"]').val().trim();

            if (catatan === '') {
                e.preventDefault();
                $checkbox.prop('checked', false);
                alert('⚠️ Please fill in the CSDL notes before validating!');
                return false;
            }
