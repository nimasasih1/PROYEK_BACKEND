...
<div class="container faq-wrapper mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 title="Kesan dan Pesan dari Mahasiswa"><i class="bi bi-chat-left-text me-2 text-primary"></i>Student Testimonials</h4>
    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content" style="border-radius: 15px;">
                <div class="modal-header border-bottom-0">
                    <h5 class="modal-title text-primary" id="formTitle" title="Tulis Kesan Baru">Write New Testimonial</h5>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold" title="Nama Mahasiswa">Student Name</label>
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold" title="Tanggal Penulisan">Date</label>
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold" title="Tulis pengalaman berkesan Anda">Testimonial</label>
                            <textarea name="kesan" id="input_kesan" class="form-control" rows="5"
                                placeholder="Share your memorable experience..." required></textarea>
                        </div>
                    </div>
                    <div class="modal-footer border-top-0">
                        <button type="button" class="btn btn-outline-secondary me-2" data-bs-dismiss="modal" title="Batal dan tutup">Cancel</button>
                        <button type="submit" class="btn btn-primary" id="submitBtn" title="Simpan kesan baru">Save Testimonial</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="faq-controls">
                <form action="{{ route('kesan.toggle', $k->id) }}" method="POST" class="d-inline">
                    ...
                    <button type="button" class="btn status-btn {{ $k->status == 1 ? 'btn-outline-secondary' : 'btn-outline-success' }}"
                        ...
                        title="{{ $k->status == 1 ? 'Sembunyikan kesan ini' : 'Tampilkan kesan ini ke publik' }}">
    ...
    <div class="text-center py-5">
        ...
        <p class="mt-2 text-muted">No testimonials have been written yet.</p>
    </div>
    @endforelse
</div>
...
<div class="modal fade" id="editKesanModal" tabindex="-1" aria-labelledby="editKesanLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <form id="editKesanForm" method="POST">
            ...
            <div class="modal-content" style="border-radius: 15px;">
                <div class="modal-header border-bottom-0">
                    <h5 class="modal-title text-primary" id="editKesanLabel" title="Edit Kesan Mahasiswa">
                        <i class="bi bi-pencil-square me-2"></i>Edit Student Testimonial
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold" title="Isi Kesan dan Pesan">Testimonial</label>
                ...
                <div class="modal-footer border-top-0">
                    <button type="button" class="btn btn-outline-secondary me-2" data-bs-dismiss="modal" title="Batal dan tutup">Cancel</button>
                    <button type="submit" class="btn btn-primary" title="Simpan perubahan kesan">Update Testimonial</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    function showAddForm() {
        document.getElementById('formTitle').innerHTML = '<i class="bi bi-plus-circle me-2"></i>Write New Testimonial';
        document.getElementById('mainForm').action = "{{ route('kesan.store') }}";
        document.getElementById('formMethod').value = "POST";
        document.getElementById('submitBtn').innerText = 'Save Testimonial';
