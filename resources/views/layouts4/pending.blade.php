...
<div class="container-fluid">
    ...
    @php
        $belumCeklis = $data->where('is_valid_fakultas', 0)->count();
    @endphp
        ...
        <table id="wisudaPendingTable" class="table table-bordered" style="width:100%; border-collapse: collapse;">
            <thead style="background-color: #8B0000;">
                <tr>
                    ...
                    <th style="color: white; padding: 12px 15px; text-align: center; font-weight: 600; border: 1px solid #8B0000; white-space: nowrap;" title="FAKULTAS ✓">FACULTY ✓</th>
                    <th style="color: white; padding: 12px 15px; text-align: center; font-weight: 600; border: 1px solid #8B0000; white-space: nowrap;" title="CATATAN FAKULTAS">FACULTY NOTES</th>
            ...
            <tbody>
                @foreach ($data->unique('id_pendaftaran') as $pendaftaran)
                    @if(!$pendaftaran->is_valid_fakultas)
                    <tr style="background-color: white;">
                        ...
                        <td style="padding: 15px; border: 1px solid #dee2e6;">
                            <form action="{{ route('layouts4.validation_faculty.update', $pendaftaran->id_pendaftaran) }}" method="POST" class="d-flex align-items-start gap-2" style="width: 500px;">
                                ...
                                <textarea name="catatan_fakultas" class="form-control" rows="2" placeholder="Write faculty notes..." title="Tulis catatan fakultas..." style="flex:2; min-width: 350px;">{{ $pendaftaran->catatan_fakultas ?? '' }}</textarea>
                            ...
                            <div class="modal fade" id="catatanFakultasModal{{ $pendaftaran->id_pendaftaran }}" tabindex="-1" aria-hidden="true">
                                    ...
                                    <div class="modal-content">
                                        <form action="{{ route('layouts4.validation_faculty.update', $pendaftaran->id_pendaftaran) }}" method="POST">
                                            ...
                                            <div class="modal-header">
                                                <h5 class="modal-title" title="Catatan Fakultas">Faculty Notes</h5>
...
<script>
    $(function() {
        ...
        // Cegah checkbox kalau catatan fakultas belum diisi
        $(document).on('change', 'input[name="is_valid_fakultas"]', function(e) {
            var $checkbox = $(this);
            var $row = $checkbox.closest('tr');
            var catatan = $row.find('textarea[name="catatan_fakultas"]').val().trim();

            if (catatan === '') {
                e.preventDefault();
                $checkbox.prop('checked', false);
                alert('⚠️ Please fill in the faculty notes before validating!');
                return false;
            }
