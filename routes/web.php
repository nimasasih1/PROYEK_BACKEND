...
->middleware('throttle:5,1'); // ← maksimal 5x coba per menit
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/beranda', function () {
        $info = \App\Models\Informasi::latest()->first();
        $kesan = \App\Models\KesanModel::where('status', 1)->latest('tanggal')->get();
        $allInfo = \App\Models\Informasi::all();
        $qna = \App\Models\Qna::orderBy('id_qna', 'asc')->get();
        return view('beranda', compact('info', 'kesan', 'allInfo', 'qna'));
    })->name('beranda');
...
Route::get('/notifikasi/baca', [NotifikasiController::class, 'baca'])
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/toga-selesai', function () { return view('viewmahasiswa.togaselesai'); })->name('toga.selesai');
...
Route::middleware(['auth'])->group(function () {
    Route::get('/layouts5', [DashboardController::class, 'index5'])->name('dashboard.index5');
    Route::get('/layouts5/validation-csdl', [DaftarWisudaController::class, 'listWisuda5'])->name('layouts5.validation_csdl.index');
    Route::get('/layouts5/pending', [DaftarWisudaController::class, 'pending5'])->name('layouts5.pending');
    Route::put('/layouts5/validation-csdl/{id}', [DaftarWisudaController::class, 'update'])->name('layouts5.validation_csdl.update1');
    Route::get('/beranda_csdl', fn() => view('beranda_csdl'))->name('beranda.csdl');
});
...
Route::get('/forgot-password/reset', fn() => view('auth.reset_password'))->name('forgot.resetForm');
